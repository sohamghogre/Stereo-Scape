export const __init__ = (canvas) => {
	...
	let config = {
		...
		BLOOM_INTENSITY: 1.33,
		BLOOM_THRESHOLD: 0.17,
		BLOOM_SOFT_KNEE: 0.7,
		SUNRAYS: true,
...
`
	...
	// 	if (x % 10 === 0) {
	// 		let pointer = pointers[0];
	// 		let posX = scaleByPixelRatio(window.innerWidth / 2);
	// 		let posY = scaleByPixelRatio(window.innerHeight / 2);
	...
	// const bangBangButtons = document.querySelectorAll('.bang-bang')
	// bangBangButtons.forEach(btn => btn.addEventListener('click', ()=>{
	...
	function updatePointerDownData(pointer, id, posX, posY) {
		...
		pointer.deltaY = correctDeltaX(pointer.texcoordX - pointer.prevTexcoordX);
		pointer.color = generateColor();
		pointer.moved = Math.abs(100);
	}
	...
	const cb = (rain=1) => {
		splatStack.push(parseInt(Math.random() * 1) + rain);
	};
	return cb;
};
